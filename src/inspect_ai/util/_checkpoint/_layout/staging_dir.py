...
checkpoints dir (``sample.json``, ``restic/host/``,
...
def restic_dir(sample_root: str) -> str:
    ...
    Contains ``host/`` and ``sandboxes/<name>/`` restic repos.
    """
...
def sample_manifest_path(sample_root: str) -> str:
    """Path to the per-sample manifest file (``<sample_root>/sample.json``).

    Carries the restic password plus the optional ``solver_done``
    marker; see :class:`SampleManifest`.
    """
    return f"{sample_root}/sample.json"
