...
class Attempt(str, Enum):
    ...
    RETRY_FOR_SCORING = "retry_for_scoring"
    """Retry of a sample whose prior attempt completed the agent loop
    but crashed during (or before) scoring. The on-disk
    ``solver_done`` marker is set. Agents that support this fast-path
    should restore their tracked state and return immediately — no
    further agent work."""
...
class ResumeCheckpoint:
    ...
    resume; the sample source reads ``solver_done`` from the on-disk
    ``sample.json`` manifest to decide which.
    """
...
class Checkpointer(Protocol):
    ...
    def attempt(self) -> Attempt:
        ...
        - :attr:`Attempt.RETRY` — prior agent loop crashed; framework
          state has been rehydrated, agent continues from where it
          left off.
        - :attr:`Attempt.RETRY_FOR_SCORING` — prior agent loop
